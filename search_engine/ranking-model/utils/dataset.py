...
class RankingDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        target = torch.tensor(
            [1.0] + [0.0] * (self.num_negative_samples - 2), dtype=torch.float
        )
        return sample, target


class DevDataset(Dataset):
    def __init__(self, parquet_path, num_negative_samples: int = 100):
        self.lf = pl.read_parquet(parquet_path)
        self.num_negative_samples = num_negative_samples

    def _create_neg_example_names(self, num_negative_samples, match: int | None = None):
        return list(range(1, num_negative_samples + 1))
    ...
    def __getitem__(self, idx):
        ...
        sample = self._extract_data(row).squeeze(-1)
        target = torch.tensor(
            [1.0] + [0.0] * (self.num_negative_samples), dtype=torch.float
        )
        return sample, target
